...
// Fragment shader from github.com/zavalit/bayer-dithering-webgl-demo (adapted)
const FRAGMENT_SRC = `
...
function pickThemeColor(): string {
  try {
    ...
    const blast = css.getPropertyValue("--pixel-blast-color").trim();
    return blast || "#08d9d6";
  } catch {
    return "#08d9d6";
  }
...
export default function PixelBlast({
  ...
  useEffect(() => {
    ...
    const canvas = document.createElement("canvas");
    canvas.className = styles.pixelBlastCanvas;
    ...
    const uniforms = {
      ...
      uTime: { value: 0 },
      uColor: { value: new THREE.Color(pickThemeColor()) },
      uClickPos: {
    ...
    const mapToPixels = (clientX: number, clientY: number) => {
    ...
    const onPointerDown = (e: PointerEvent) => {
      const { fx, fy } = mapToPixels(e.clientX, e.clientY);
    ...
    const mo = new MutationObserver(() => {
      uniforms.uColor.value.set(pickThemeColor());
    });
  ...
  return (
    <div
      ref={containerRef}
      className={[styles.pixelBlastContainer, className].filter(Boolean).join(" ")}
      style={style}
      aria-hidden
    />
  );
